# coding: utf-8

"""
    Authentication

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from tl_pax8.models.create_line_item import CreateLineItem

class TestCreateLineItem(unittest.TestCase):
    """CreateLineItem unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> CreateLineItem:
        """Test CreateLineItem
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `CreateLineItem`
        """
        model = CreateLineItem()
        if include_optional:
            return CreateLineItem(
                id = '',
                product_id = '',
                commitment_term_id = '',
                provision_start_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                line_item_number = 1,
                billing_term = 'Monthly',
                parent_subscription_id = '',
                parent_line_item_number = 1,
                quantity = 45,
                provisioning_details = [
                    tl_pax8.models.provisioning_detail.ProvisioningDetail(
                        label = '', 
                        key = 'userEmailAddress', 
                        values = [
                            ''
                            ], 
                        description = 'Provide the user's email address', 
                        value_type = 'Input', 
                        possible_values = [
                            ''
                            ], )
                    ]
            )
        else:
            return CreateLineItem(
                product_id = '',
                line_item_number = 1,
                billing_term = 'Monthly',
                quantity = 45,
        )
        """

    def testCreateLineItem(self):
        """Test CreateLineItem"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
